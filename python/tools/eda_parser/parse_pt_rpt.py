...
class parse_icc_qor:
    ...
    def read_icc_qor:
        ...
        design = pp.Group("Design" + flatName)
        version = pp.Group("Version" + flatName)
        sceTimGroup = pp.Group("Scenario" + SQUOTA + flatName + SQUOTA +
                                 "Timing Path Group" + SQUOTA + pathGroupName + SQUOTA +
                                 pp.OneOrMore(DASH) +
                                 ...
                                 "No. of Violating Paths" + COLON + float +
                                 pp.OneOrMore(DASH)
                                 )
        cellCount = pp.Group(
            "Cell Count" + dashLine +
        ...
        area = pp.Group("Area" + dashLine +
            ...
            "Net Area:" + float +
            "Net XLength:" + float +
            "Net YLength:" + float + dashLine +
            "Cell Area (netlist):" + float +
            "Cell Area (netlist and physical only):        70319.33"     + float +
            "Net Length:" + float
        )

        designRule =pp.Group("Design Rules" + dashLine +
                             "Total Number of Nets:" + integer +
                             "Nets with Violations:" + integer +
                             "Max Trans Violations:" + integer +
                             "Max Cap Violations:" + integer +dashLine)
